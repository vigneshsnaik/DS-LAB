...
int stack[MAXSIZE], tempStack[MAXSIZE], top = -1, tempTop = -1;
    ...
    while (1)
    {
        ...
        if (!strcmp("push", cmd))
        {
            ...
            while (temp != NULL)
            {
                ...
                if (atoi(temp))
                {
                    while (!isEmpty(stack, &top) && atoi(temp) < peek(stack, &top))
                    {
                        push(tempStack, &tempTop, pop(stack, &top), MAXSIZE);
                    }
                    printf(GRN "%d Pushed into stack\n", push(stack, &top, atoi(temp), MAXSIZE));
                    while (!isEmpty(tempStack, &tempTop))
                    {
                        push(stack, &top, pop(tempStack, &tempTop), MAXSIZE);
                    }
                }
                temp = strtok(NULL, ",");
            }
